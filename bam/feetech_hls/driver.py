...
class FeetechHLSDriver:
    ...
    def set_goal_position(self, servo_id: int, position_rad: float) -> int:
        """Set a signed target [rad] relative to calibrated center position 2048."""
        raw_value = int(
            round(position_rad / self.POSITION_RAD_PER_LSB)
        ) + self.POSITION_CENTER_LSB
        if not 0 <= raw_value < 4096:
            raise ValueError(
                f"Position target {position_rad:.3f} rad is outside HLS's calibrated range"
            )
        return self.write_bytes(
            servo_id, self.ADDR_GOAL_POSITION, [raw_value & 0xFF, raw_value >> 8]
        )

    def set_position_kp(self, servo_id: int, kp: int) -> int:
        """Set the Position-mode firmware proportional gain (register 0x32)."""
        if not 0 <= kp <= 0xFF:
            raise ValueError(f"Position Kp must be in [0, 255], got {kp}")
        return self.write_bytes(servo_id, self.ADDR_KP, [kp])
    ...
    def read_status(self, servo_id: int = 1) -> Dict[str, Any]:
        ...
        # Position (0x38) is a 0--4095 absolute encoder. Position-mode
        # calibration assigns the physical vertical to POSITION_CENTER_LSB, so
        # the recording coordinate stays continuous through zero.
        raw_pos = raw[0] | (raw[1] << 8)
        pos_rad = (raw_pos - self.POSITION_CENTER_LSB) * self.POSITION_RAD_PER_LSB
